/**
 * Converts UI filter state to GraphQL-compatible format.
 * Transforms height/weight classes to numeric ranges and maps sort options.
 */
...
export function usePokemonFilters({
  ...
  // Build GraphQL filter object from UI state
  const graphqlFilter = useMemo(() => {
    ...
    // Convert height classes to numeric ranges
    ...
    // Convert weight classes to numeric ranges  
  ...
  // Map UI sort options to GraphQL format
